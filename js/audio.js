...
export const AudioEngine = (() => {
  let ctx = null;
  let master = null, reverbBus = null;
  ...
  /* ---------- 弦乐群：多失谐锯齿 + 低通 + 缓慢起音 ---------- */
  function stringNote(freq, t, dur, vel, filt) {
    const oscs = [];
  ...
  function playStep(stepIndex, t) {
    ...
    if (stepInBar === 6 || stepInBar === 14) {
      const cIdx = barIndex * 2 + (stepInBar === 6 ? 0 : 1);
  ...
  function noiseHit(dur, vol, hp, lp, delay) {
    if (!sfxBus) return;
    const t = ctx.currentTime + (delay || 0);
  ...
  const SFX = {
    ...
    unlock:  () => { [523, 659, 784, 1046].forEach((f, i) => tone(f, 0.22, 'sine', 0.13, 0, i * 0.09)); noiseHit(0.5, 0.08, null, 2600, 0.3); },
    curse:   () => { tone(220, 0.5, 'sawtooth', 0.1, 140); tone(110, 0.6, 'sine', 0.12, 70, 0.05); },
    pickup:  () => tone(1180, 0.07, 'triangle', 0.09, 1520),
    hurt:    () => { tone(180, 0.22, 'sawtooth', 0.16, 90); noiseHit(0.16, 0.1, 2400, null); },
    kill:    () => { noiseHit(0.18, 0.12, 3000, null); tone(260, 0.16, 'sawtooth', 0.08, 90); },
    hit:     () => { const t = ctx ? ctx.currentTime : 0; if (t - lastHitAt < 0.06) return; lastHitAt = t; noiseHit(0.06, 0.07, 3600, null); },
    w_moon:     () => noiseHit(0.3, 0.1, null, 1400),
    w_crossbow: () => { noiseHit(0.08, 0.12, 5000, null); tone(1500, 0.07, 'triangle', 0.07, 1100); },
    w_arc:      () => { noiseHit(0.18, 0.14, null, 2000); tone(880, 0.2, 'sawtooth', 0.08, 220); },
    w_meteor:   () => { tone(160, 0.55, 'sawtooth', 0.12, 60); noiseHit(0.5, 0.1, null, 700, 0.1); },
    ...
    w_lance:    () => { tone(520, 0.16, 'triangle', 0.11, 260); noiseHit(0.12, 0.09, 3200, null, 0.04); },
    w_shadow:   () => { tone(300, 0.24, 'sawtooth', 0.09, 120); tone(150, 0.3, 'sine', 0.08, 80, 0.06); },
    w_storm:    () => noiseHit(0.2, 0.08, null, 1800),
    w_nova:     () => { tone(440, 0.3, 'sawtooth', 0.12, 220); noiseHit(0.28, 0.12, null, 1500, 0.02); [660, 880, 1320].forEach((f, i) => tone(f, 0.2, 'triangle', 0.08, 0, 0.05 + i * 0.05)); },
    w_phantom:  () => { [880, 660, 440].forEach((f, i) => tone(f, 0.3, 'sine', 0.08, f * 0.5, i * 0.12)); },
    boss_wave:  () => { tone(300, 0.3, 'sawtooth', 0.12, 700); noiseHit(0.22, 0.1, null, 1800); },
