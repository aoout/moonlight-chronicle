...
export class CombatSystem extends System {
  ...
  /** 静态委托：击杀敌人 */
  static killEnemy(e: any, srcType?: string): void { return killEnemy(e, srcType); }
  /** 静态委托：玩家受伤 */
  static hurtPlayer(e: any, rawDmg: number): void { return hurtPlayer(e, rawDmg); }
  /** 静态委托：治疗玩家 */
  static healPlayer(n: number): void { return healPlayer(n); }
  /** 静态委托：伤害敌人 */
  static damageEnemy(e: any, dmg: number, isCrit: boolean, srcType?: string, srcW?: string): void { return damageEnemy(e, dmg, isCrit, srcType, srcW); }
  /** 静态委托：近战打击 */
  static meleeHit(x: number, y: number, r: number, dmg: number, opts?: { mul?: number; shake?: number }): void { return meleeHit(x, y, r, dmg, opts); }
}
...
export function calcDamage(base: number, p: Player): { dmg: number; crit: boolean } {
...
export function killEnemy(e: any, srcType?: string): void {
  if (e.dead) return;
  e.dead = 1;
  sSt().kills++;
...
export function hurtPlayer(e: any, rawDmg: number): void {
  ...
  if (p.thorns > 0 && e && 'hp' in e) damageEnemy(e, dmg * p.thorns, false, 'thorns');
