...
/**
 * 执行武器开火管线
 * @param {object} w 武器实例 {id, lv}
 * @param {object} p 玩家
 * @param {object} def 武器定义（来自 WEAPONS）
 * @param {number} lv 武器等级
 * @param {number} baseDmg 基础伤害
 * @returns {boolean} 是否成功开火
 */
export function executeFirePipeline(w, p, def, lv, baseDmg) {
...
/* 开火特效 */
function spawnFireEffects(p, def, wId) {
 ...
 * @param {object} p 玩家
 */
export function executeProjPipeline(pr, dt, p) {
...
/* 尾迹特效 */
function spawnTrailFx(pr, dt, moveType) {
  if (pr.dead) return;
