 ...
 */
export const MOVEMENT: Record<string, (pr: any, dt: number, p: any) => boolean> = {
  ...
  boomerang(pr, dt, p) {
    ...
    pr.x = tx; pr.y = ty;
    if (!pr.ret && pr.t * pr.speed >= (pr.range || 420) * 0.6) pr.ret = true;
    if (pr.ret) {
      const a = Math.atan2(p.y - pr.y, p.x - pr.x);
      pr.dir = a;
      pr.x += Math.cos(a) * pr.speed * dt;
      pr.y += Math.sin(a) * pr.speed * dt;
      if (dist(pr, p) < 26) { pr.dead = true; return false; }
    }
    return true;
  },
  ...
  breath(pr, dt, _p) {
    pr.t = (pr.t || 0) + dt;
    if (pr.t > (pr.dur || 0.6)) { pr.dead = true; return false; }
    return true;
  },
};
