...
/** @type {import('../types/core.d.ts').Config} */
export const CONFIG = {
...
/** @type {import('../types/core.d.ts').LevelDef[]} */
export const LEVELS = [
...
/** @type {import('../types/core.d.ts').CurseDef[]} */
export const CURSES = [
  { id:'curse_price',   name:'蚀雾弥漫', icon:ICONS.coin,   desc:'集市货价 +30%',          apply:p=>{ p._priceMul = 1.3; } },
  ...
  { id:'curse_ehp',     name:'蚀潮沸腾', icon:ICONS.triUp,  desc:'敌人生命 +25%',          apply:p=>{ p._enemyHpMul = 1.25; } },
  { id:'curse_edmg',    name:'蚀牙锋利', icon:ICONS.flame,  desc:'敌人伤害 +15%',          apply:p=>{ p._enemyDmgMul = 1.15; } },
];
...
/** @param {number} stage */
export function stageEnemyPool(stage){
  const pool = ['grub','rat'];
...
/** @param {number} stage */
export function inflationRate(stage) {
  const t = (stage - 1) / 18;
  return 1 + 2 * t * t;
}

/** @param {number} stage */
export function stageSpawnRate(stage){
  return 1.1 + stage * 0.18;
}

/** @param {number} stage */
export function enemyScale(stage){
  const hp = 1 + (stage - 1) * 0.32 + (stage - 1) * (stage - 1) * 0.012;
  const dmg = 1 + (stage - 1) * 0.12;
  return { hp, dmg };
}