...
/**
 * 公式解析器
 * @param {string|number} formula - 公式字符串或数值常量
 ...
 * @returns {{type:string, raw:string, pos:number, value?:number, name?:string}[]}
 */
function tokenize(formula) {
  /** @type {{type:string, raw:string, pos:number, value?:number, name?:string}[]} */
  const tokens = [];
  let i = 0;
