...
const BOSS_SKILLS: Record<string, (e: any) => void> = {
  ...
  tidalWave(e: any) {
    for (let w = 0; w < 3; w++) {
      ...
      for (let j = 0; j < 12; j++) {
        ...
        eSt().projectiles.push(PROJECTILE_POOL.addWith({ x: e.x, y: e.y, vx: Math.cos(a) * (170 + w * 75), vy: Math.sin(a) * (170 + w * 75), r: 5, dmg: e.dmg * 0.45, color: '#5c8a9e', hit: new Set(), enemy: true, life: 3 }));
      }
    }
  },
  /* 潮噬之母：产卵孵化潮虫群 */
  ...
  moonSlash(e: any) {
    ...
    for (let i = -2; i <= 2; i++) {
      const ang = a + i * 0.35;
      eSt().projectiles.push(PROJECTILE_POOL.addWith({ x: e.x, y: e.y, vx: Math.cos(ang) * 380, vy: Math.sin(ang) * 380, r: 8, dmg: e.dmg * 0.8, color: '#b49ae8', hit: new Set(), enemy: true, life: 1.2 }));
    }
  },
  /* 月影巫王：暗影追踪球 + 诅咒减速 */
  shadowOrb(e: any) {
    const p = pSt().player; if (!p) return;
    for (let i = 0; i < 3; i++) {
      eSt().projectiles.push(PROJECTILE_POOL.addWith({ homing: true, x: e.x, y: e.y, vx: 0, vy: 0, target: p, speed: 165, r: 8, dmg: e.dmg * 0.8, color: '#9a86c8', hit: new Set(), enemy: true, t: 0, life: 4 }));
    }
  ...
  triSlash(e: any) {
    ...
    for (let w = 0; w < 3; w++) {
      for (let i = -3; i <= 3; i++) {
        ...
        eSt().projectiles.push(PROJECTILE_POOL.addWith({ x: e.x + Math.cos(a) * 40, y: e.y + Math.sin(a) * 40, vx: Math.cos(ang) * 430, vy: Math.sin(ang) * 430, r: 7, dmg: e.dmg * 0.7, color: '#c8c2e8', hit: new Set(), enemy: true, life: 1.1 }));
      }
    }
  },
  /* 裂空魔龙：烈焰吐息（锥形持续） */
  ...
  /* 深渊巢母：酸雾（扩散减速区域） */
  acidMist(e: any) {
    eSt().projectiles.push(PROJECTILE_POOL.addWith({ aoe: true, x: e.x, y: e.y, r: 0, maxR: 360, dmg: e.dmg * 0.2, color: '#7fce5a', t: 0, slow: 0.5, enemy: true, hit: new Set() }));
  },
  /* 蚀月终焉：蚀月脉冲（全向扩散弹幕） */
  eclipsePulse(e: any) {
    for (let j = 0; j < 14; j++) {
      const a = (j / 14) * 6.28;
      eSt().projectiles.push(PROJECTILE_POOL.addWith({ x: e.x, y: e.y, vx: Math.cos(a) * 210, vy: Math.sin(a) * 210, r: 6, dmg: e.dmg * 0.6, color: '#ffb84d', hit: new Set(), enemy: true, life: 3 }));
    }
  },
};

export function bossTick(e: any, dt: number): void {
  const p = pSt().player;
  if (!p) return;
  e.stateT -= dt;
  e.attT -= dt;
...
export function bossWave(e: any): void {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    eSt().projectiles.push(PROJECTILE_POOL.addWith({ enemy: true, x: e.x, y: e.y, vx: Math.cos(ang) * 150, vy: Math.sin(ang) * 150, r: 7, dmg: e.dmg * 0.7, life: 3, color: '#ff9d6b', pierce: 0, hit: new Set() }));
  }
  shakeScreen(4);
}
export function bossMinions(e: any): void {
  for (let i = 0; i < 3; i++) SpawnSystem.spawnEnemy(pick(['grub', 'rat', 'wing', 'charger']), { hpMul: 0.7 });
}
export function bossDash(e: any): void {
  const p = pSt().player; if (!p) return;
  const a = angTo(e, p);
