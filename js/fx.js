...
/* 扩散圆环（冲击波） */
export function spawnRing(x, y, color, max, r1, lw) {
  addFx({ ring: true, x, y, t: 0, max: max || 0.5, color, r0: 4, r1: r1 || 60, lw: lw || 3 });
}
/* 方向火花（短线段） */
export function spawnSpark(x, y, color, n, sp) {
  for (let i = 0; i < n; i++) {
...
/* 星形爆点 */
export function spawnStar(x, y, color, size) {
  addFx({ star: true, x, y, t: 0, max: 0.4, size: size || 10, color });
}
/* 旋转碎片（菱形，旋转飞出） */
export function spawnShard(x, y, color, n, sp) {
  for (let i = 0; i < n; i++) {
