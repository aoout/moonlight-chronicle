...
function initBackgroundCache() {
  if (_bgCache && _bgCacheW === G.width && _bgCacheH === G.height) return;
  _bgCache = document.createElement('canvas');
  _bgCache.width = G.width;
  _bgCache.height = G.height;
  _bgCacheW = G.width;
  _bgCacheH = G.height;
  const bc = /** @type {CanvasRenderingContext2D} */ (_bgCache.getContext('2d'));

  // 月面底色（俯视：月灰蓝渐变）
  const grd = bc.createRadialGradient(G.width / 2, G.height / 2, 60, G.width / 2, G.height / 2, Math.max(G.width, G.height) * 0.72);
  grd.addColorStop(0, '#262c48');
  grd.addColorStop(0.55, '#151a30');
  grd.addColorStop(1, '#0b0e1e');
  bc.fillStyle = grd;
  bc.fillRect(0, 0, G.width, G.height);

  // 月光方向（左上受光 → 右下微暗）
  const light = bc.createLinearGradient(0, 0, G.width, G.height);
  ...
  // 月海（暗色平滑区）
  for (let i = 0; i < 4; i++) {
    const hx = ((i * 137.5 + 40) % 100) / 100 * G.width;
    const hy = ((i * 89.7 + 20) % 100) / 100 * G.height;
    const hr = 90 + i * 55;
  ...
  for (let i = 0; i < 14; i++) {
    const cx = ((i * 61.7 + 9) % 100) / 100 * G.width;
    const cy = ((i * 43.3 + 55) % 100) / 100 * G.height;
    const cr = 8 + (i * 7.3 % 26);
  ...
  bc.lineWidth = 1;
  for (let i = 0; i < 6; i++) {
    const bx = ((i * 173 + 30) % 100) / 100 * G.width;
    const by = ((i * 97 + 12) % 100) / 100 * G.height;
    bc.beginPath();
    bc.moveTo(bx, by);
    let px = bx, py = by;
    const segs = 3 + i % 3;
...
export function drawBackground() {
  const ctx = /** @type {CanvasRenderingContext2D} */ (G.ctxBg);
  initBackgroundCache();
  ...
  ctx.save();
  for (let i = 0; i < 60; i++) {
    const dx = (i * 91.7 + 3) % G.width;
    const dy = (i * 53.3 + 71) % G.height;
    const tw = 0.5 + 0.5 * Math.sin(G.time * 0.8 + i * 2.3);
