...
**步骤：**
- [ ] 1.1 创建 `js/ecs/World.js` — ECS World 类
  - 封装所有 EntityPool，提供 `add()`, `query()`, `compact()` API
  - 统一管理实体生命周期
- [ ] 1.2 改造 `System` 基类 — 支持依赖注入
  - 构造函数接收 `(world, eventBus, config)`
  - 提供 `this.world`, `this.eventBus`, `this.config` 属性
- [ ] 1.3 改造 `SystemManager` — 依赖注入容器
  - 创建 World 实例
  - 在 `add()` 时自动注入依赖
- [ ] 1.4 验证：`npm run build` 零错误，`npm test` 全绿
...
**步骤：**
- [ ] 3.1 迁移简单系统（StageTimerSystem, BossCheckSystem, SpatialSystem）
- [ ] 3.2 迁移中等系统（DropSystem, ParticleSystem, OrbitSystem）
- [ ] 3.3 迁移复杂系统（EnemySystem, SpawnSystem, ProjectileSystem）
- [ ] 3.4 迁移 CombatSystem（事件驱动，复杂副作用）
- [ ] 3.5 迁移 PlayerSystem（最复杂系统）
- [ ] 3.6 更新 `game.js` 入口 — 使用新的 SystemManager
- [ ] 3.7 验证：`npm run build` 零错误，`npm test` 全绿
...
- [ ] 4.1 创建渲染数据接口定义
- [ ] 4.2 重构渲染函数，通过参数接收数据
- [ ] 4.3 更新 `render/index.js` 调度
- [ ] 4.4 验证：`npm run build` 零错误
...
**步骤：**
- [ ] 5.1 拆分 `ui/shop.js` (494行) → `ui/shop/` 目录
- [ ] 5.2 拆分 `render/effects.js` (356行) → `render/effects/` 目录
- [ ] 5.3 验证：`npm run build` 零错误，游戏功能完整
