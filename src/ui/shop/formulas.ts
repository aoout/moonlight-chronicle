...
const WEAPON_FORMULAS: Record<string, FormulaEntry[]> = {
  ...
  frost:      [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.7+0.12*lv,'(0.7+0.12×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.7+0.12*lv)], ['生命上限','param',p=>p.maxHp||0], ['生命倍率','param',(p,lv)=>0.10*(1+lv*0.2),'(10%+2%×L)'], ['生命·倍率','part',(p,lv)=>(p.maxHp||0)*0.10*(1+lv*0.2)]],
  ...
  orbit:      [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>(0.4+0.1*lv)*6,'(0.4+0.1×L)×6'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.4+0.1*lv)*6]],
  ...
  shadow:     [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.7+0.13*lv,'(0.7+0.13×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.7+0.13*lv)], ['溢出暴伤','param',p=>Math.max(0,(p.critDmg||1.5)-1.5)], ['溢出暴伤·攻·0.9','part',(p,lv)=>Math.max(0,(p.critDmg||1.5)-1.5)*(p.effAtk||0)*0.9]],
  storm:      [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.5+0.09*lv,'(0.5+0.09×L)'], ['单发伤害','part',(p,lv)=>(p.effAtk||0)*(0.5+0.09*lv)], ['移速','param',p=>p.speed||0], ['攻速','param',p=>p.atkSpd||1], ['移速·攻速·0.02','part',(p,lv)=>(p.speed||0)*(p.atkSpd||1)*0.02], ['每核弹幕数','param',(p,lv)=>1+Math.floor(lv/2)], ['双核·总输出','part',(p,lv)=>((p.effAtk||0)*(0.5+0.09*lv)+(p.speed||0)*(p.atkSpd||1)*0.02)*(1+Math.floor(lv/2))*2]],
