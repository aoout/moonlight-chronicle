...
import { ENEMIES, BOSSES } from '../../data/index.js';
import { PROJECTILE_POOL, PARTICLE_POOL } from '../../ecs/entity_pool.js';
...
/** 生成一个投射物 */
function spawnProjectile(x: number, y: number, vx: number, vy: number): void {
...
function ensurePlayer(): void {
  if (!pSt().player) {
    const dummy = {
      ...
      r: 20, hp: 100, maxHp: 100, invuln: 0,
      facing: 0, spd: 180,
      dmg: 10, dmgMul: 1, atkSpd: 1,
      range: 300, speed: 200, atk: 10,
      pierce: 0, size: 20, crit: 0,
      armor: 0, dodge: 0, regen: 0, lifeSteal: 0,
      weapons: [], effects: { weapons: [], items: [], itemStats: {} },
    };
    playerState.set('player', dummy as any);
  }
}
