...
const WEAPON_FORMULAS: Record<string, FormulaEntry[]> = {
  ...
  phantom:    [['攻击力','param',p=>p.effAtk||0], ['分身数','param',(p,lv)=>2+Math.floor(lv/2)], ['单分身·攻击×(0.8+0.12L)','param',(p,lv)=>(p.effAtk||0)*(0.8+0.12*lv)], ['总输出(单×分身数)','part',(p,lv)=>(p.effAtk||0)*(0.8+0.12*lv)*(2+Math.floor(lv/2))]],
