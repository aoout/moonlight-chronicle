...
const WEAPON_FORMULAS: Record<string, FormulaEntry[]> = {
  moonRing:   [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.55+0.12*lv,'(0.55+0.12×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.55+0.12*lv)], ['移速','param',p=>p.speed||0], ['移速倍率','param',(p,lv)=>0.12+0.02*lv,'(0.12+0.02×L)'], ['移速·倍率','part',(p,lv)=>(p.speed||0)*(0.12+0.02*lv)]],
  ...
  meteor:     [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>2.8+0.5*lv,'(2.8+0.5×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(2.8+0.5*lv)], ['生命上限','param',p=>p.maxHp||0], ['生命·13%','part',(p,lv)=>(p.maxHp||0)*0.13]],
  frost:      [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.7+0.12*lv,'(0.7+0.12×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.7+0.12*lv)], ['生命上限','param',p=>p.maxHp||0], ['生命倍率','param',(p,lv)=>0.16+0.024*lv,'(16%+2.4%×L)'], ['生命·倍率','part',(p,lv)=>(p.maxHp||0)*(0.16+0.024*lv)]],
  ...
  orbit:      [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>(0.8+0.2*lv)*6,'(0.8+0.2×L)×6'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.8+0.2*lv)*6]],
  ...
  shadow:     [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>1.1+0.15*lv,'(1.1+0.15×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(1.1+0.15*lv)], ['溢出暴伤','param',p=>Math.max(0,(p.critDmg||1.5)-1.5)], ['溢出暴伤·攻·0.9','part',(p,lv)=>Math.max(0,(p.critDmg||1.5)-1.5)*(p.effAtk||0)*0.9]],
  ...
  tideAnchor: [['攻击力','param',p=>p.effAtk||0], ['倍率','param',(p,lv)=>0.75+0.13*lv,'(0.75+0.13×L)'], ['攻击·倍率','part',(p,lv)=>(p.effAtk||0)*(0.75+0.13*lv)], ['生命上限','param',p=>p.maxHp||0], ['生命倍率','param',(p,lv)=>0.12+0.015*lv,'(12%+1.5%×L)'], ['生命·倍率','part',(p,lv)=>(p.maxHp||0)*(0.12+0.015*lv)], ['月蚀深度','param',()=>stageDepth(),'深度'], ['深度倍率','param',(p,lv)=>1+stageDepth()*0.12,'(1+深度×12%)'], ['深度·倍率','part',(p,lv)=>((p.effAtk||0)*(0.75+0.13*lv)+(p.maxHp||0)*(0.12+0.015*lv))*(stageDepth()*0.12)], ['基准伤害','part',(p,lv)=>((p.effAtk||0)*(0.75+0.13*lv)+(p.maxHp||0)*(0.12+0.015*lv))*(1+stageDepth()*0.12)], ['落点冲击 ×0.6','part',(p,lv)=>((p.effAtk||0)*(0.75+0.13*lv)+(p.maxHp||0)*(0.12+0.015*lv))*(1+stageDepth()*0.12)*0.6], ['水域潮压 4跳 ×0.14','part',(p,lv)=>((p.effAtk||0)*(0.75+0.13*lv)+(p.maxHp||0)*(0.12+0.015*lv))*(1+stageDepth()*0.12)*0.14*4]],
};
...
export function weaponProjInfo(def: any, p: Player): ProjInfo {
  ...
  const map: Record<string, () => ProjInfo> = {
    ...
    storm:   ()=>{ const b=1;const e=b+Math.floor(bonus/3);return{base:b,total:Math.max(1,b+e),multi:true}; },
